...
Workflows can also carry a typed runtime context across steps. Declare an initial value in `createWorkflow({ ctx })`, then merge per-run overrides via `workflow.run({ ctx })`. The `ctx` handed to each handler is read-only; use `stepRuntime.updateCtx()` to produce the next state.
...
>({
  ...
  .then(
    createStep({
      id: "format-summary",
      handler: ({ ctx }) => `Org ${ctx.orgId} processed ${ctx.total}`,
    }),
  )
  .commit();
