...
Déclarez vos options directement dans `createWorkflow`. Placez `telemetry: true` pour un opt-in rapide, ou passez des overrides pour personnaliser le `traceName`, l'enregistrement des entrées/sorties et les métadonnées.
...
Chaque exécution héritera de cette configuration. Les attributs sont écrits avec le préfixe `ai_kit.workflow.*`, ce qui facilite la recherche dans vos backends OTEL ou Langfuse.
...
Les overrides sont fusionnés avec la configuration globale : les métadonnées sont combinées, et les drapeaux `recordInputs` / `recordOutputs` du run priment sur ceux du workflow.
