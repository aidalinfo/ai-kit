...
const checkStatus = createMapStep<{ jobId: string; attempt: number }, { done: boolean }>({
  id: "check-status",
  ...
  handler: async ({ input }) => {
    const done = await fetch(`/jobs/${input.jobId}/status?attempt=${input.attempt}`)
      .then(res => res.json())
...
const pollUntilDone = createWhileStep({
  ...
  maxIterations: 12,
  condition: ({ iteration, input, lastOutput }) => {
    const done = lastOutput?.done ?? false;
    // Tant que le job n'est pas done, on continue (12 tentatives max)
    return !done || iteration === 0;
  },
  prepareNextInput: ({ iteration, input }) => ({
    jobId: input.jobId,
    attempt: iteration,
  }),
  collect: ({ results }) => ({
...
export const pollWorkflow = createWorkflow({
  ...
  outputSchema: z.object({
    lastResult: z.object({ done: z.boolean() }).nullable(),
    attempts: z.number().int(),
    history: z.array(z.object({ done: z.boolean() })),
  }),
})
  .while(pollUntilDone)
  .commit();
...
Par défaut, le step retourne `{ lastResult, allResults }` avec `lastResult` potentiellement `undefined` si aucune itération n'a eu lieu. Ici nous passons un `collect` personnalisé pour enrichir la sortie. `maxIterations` reste obligatoire et déclenche un `WorkflowExecutionError` si la condition voudrait poursuivre au-delà de la limite : l'UI peut ainsi afficher un message explicite.
...
- `prepareNextInput` est optionnel. Sans lui, l'input initial est passé lors de la première itération puis chaque output devient l'input suivant. Utilisez ce hook pour injecter des métadonnées supplémentaires (ex. compteur d'essais).
- `collect` reçoit `{ input, results, lastResult, iterations, context }` et vous laisse façonner la sortie (aggrégation, mapping vers une structure métier).
