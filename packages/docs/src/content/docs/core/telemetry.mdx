...
Les workflows utilisent désormais la même pile OpenTelemetry. Activez-les via `telemetry` dans `createWorkflow`, la méthode `workflow.withTelemetry()` ou le décorateur `withTelemetry(workflow, config)`. Chaque run produit :

- un span racine `workflow.<id>` (override possible avec `traceName`)
...
Vous pouvez enrichir un run à la volée en passant `telemetry: { metadata: { ... } }` dans `workflow.run({ ... })`, ou le désactiver ponctuellement avec `telemetry: false`. Consultez la page [`/docs/core/workflows/telemetry`](../workflows/telemetry) pour un guide complet et des exemples Langfuse.
