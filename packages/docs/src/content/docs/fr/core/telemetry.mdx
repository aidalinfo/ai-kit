...
Vous pouvez enrichir un run à la volée en passant `telemetry: { metadata: { ... } }` dans `workflow.run({ ... })`, ou le désactiver ponctuellement avec `telemetry: false`. Consultez la page [`/docs/core/workflows/telemetry`](/core/workflows/telemetry/) pour un guide complet et des exemples Langfuse.
