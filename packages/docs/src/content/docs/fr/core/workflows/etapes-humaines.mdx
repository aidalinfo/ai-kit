...
Les étapes humaines permettent d'interrompre l'exécution d'un workflow pour attendre une décision manuelle ou un complément d'information. Elles sont déclarées via `createHuman` et peuvent être réutilisées dans plusieurs workflows.
...
- La sortie de `fetchDraftApplication` est injectée dans `manualReview` (`output.current`) et renvoyée vers le front via `pendingHuman.output`.
- La réponse humaine devient l'entrée de `finalizeDecision`, qui peut aussi accéder aux données initiales via `context.initialInput`.

## Consommer l'interaction côté runtime
...
`run.resumeWithHumanInput` valide la réponse selon le schéma de l'étape humaine puis relance l'exécution jusqu'à complétion. Ce pattern convient aux revues manuelles, validations légales ou doubles saisies nécessitant une intervention humaine tracée.

### Bonnes pratiques

- Donnez des identifiants stables à vos étapes humaines : ils servent à reprendre l'exécution.
- Exposez des métadonnées utiles dans `output` pour faciliter l'affichage côté front (résumé du dossier, liens externes, etc.).
- Enregistrez les événements `step:human:*` dans votre observabilité afin de mesurer les temps de traitement.

Besoin de paralléliser vos traitements avant ou après la revue humaine ? Découvrez [`Parallèle & foreach`](./parallele-et-foreach).
