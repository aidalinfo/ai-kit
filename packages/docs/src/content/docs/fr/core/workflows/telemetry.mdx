...
> ℹ️ Lorsque `userId` est renseigné, l’attribut est exporté sous les clés `langfuse.user.id`, `user.id` et `ai_kit.workflow.user_id`, ce qui remplit la colonne **User ID** dans l’interface Langfuse.
...
Exposez un identifiant utilisateur via `telemetry.userId` pour lier un run à un compte final. La valeur se combine avec celle déclarée à la construction (si présente) et reste accessible dans Langfuse ainsi que dans les hooks d’exécution (`getTelemetrySelection()`).
...
Dans Langfuse, l’identifiant apparaît dans les attributs de la trace (`langfuse.user.id`) et dans la colonne **User ID** du tableau des runs. Combinez-le avec des métadonnées supplémentaires (`tenantId`, `plan`, etc.) pour faciliter vos filtres.
