...
### Partager un `ctx` d'exécution

Les workflows peuvent transporter un contexte d'exécution typé entre les étapes. Déclarez une valeur initiale dans `createWorkflow({ ctx })`, puis fusionnez une surcharge par exécution via `workflow.run({ ctx })`. Le `ctx` reçu dans chaque handler est en lecture seule ; utilisez `stepRuntime.updateCtx()` pour propager un nouvel état.
